    ...
    void HandleAttacking()
    {
        ...
        if (attackTimer >= currentCooldown)
        {
            ...
            if (playerInAttackRange)
            {
                ...
                attackTimer = 0;
                firstAttack = false; // Despu�s del primer ataque, cambiar a false
            }
        }

        // Regresar a persecuci�n si jugador est� fuera de rango
        float distanceToPlayer = Vector2.Distance(transform.position, playerTransform.position);
        if (distanceToPlayer > attackDistance) // Usar la nueva variable de distancia de ataque
        {
            currentState = EnemyState.Chasing;
            firstAttack = true; // Reiniciar para el pr�ximo encuentro
        }
    }
